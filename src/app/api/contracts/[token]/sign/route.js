...
export async function POST(request, { params }) {
  ...
  const candidate = await Contract.findOne({ tokenHash }).select("+tokenHash +signature");
  ...
  const signed = await Contract.findOneAndUpdate({ _id: candidate._id, tokenHash, status: { $in: ["ready", "viewed"] }, signedAt: null, $or: [{ linkExpiresAt: null }, { linkExpiresAt: { $gt: now } }] }, { $set: { status: "signed", signedAt: now, signerName, signature, signatureMethod: method, typedSignatureConsent: true, signatureConsentText: consentText, signatureConsentVersion: 1, acceptedTerms: true, auditIp: ip, auditUserAgent: sanitizeText(request.headers.get("user-agent"), 500), integrityHash, "notification.status": "pending" }, $unset:{tokenValue:""}, $push: { audit: { event: "signed", at: now, actor: "client", detail: `${method}; template version ${candidate.templateVersion}` } } }, { new: true }).select("+signature +auditIp +auditUserAgent");
