...
export async function GET(request, { params }) {
  ...
  if (contract.status === "signed") return publicHeaders(NextResponse.json({ state: "signed" }, { status: 410 }));
  if (contract.status === "revoked") return publicHeaders(NextResponse.json({ state: "revoked" }, { status: 410 }));
  if (contract.linkExpiresAt && contract.linkExpiresAt <= new Date()) { if (contract.status !== "expired") await Contract.updateOne({ _id: contract._id, status: { $in: ["ready","viewed"] } }, { $set: { status: "expired" }, $push: { audit: { event: "expired", actor: "system" } } }); return publicHeaders(NextResponse.json({ state: "expired" }, { status: 410 })); }
